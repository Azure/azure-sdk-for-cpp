...
public:
  ...
  */
   ...
   * @return `true` if #Azure::ETag matches, `false` otherwise.
   */
  static bool Equals(
      const ETag& left,
      const ETag& right,
  ...
  /**
   * @brief Construct an empty (null) #Azure::ETag.
   */
  ETag() = default;

  /**
   * @brief Construct a #Azure::ETag.
   * @param etag The string value representation.
   */
  explicit ETag(std::string etag) : m_value(std::move(etag)) {}

  /**
   * @brief Whether #Azure::ETag is present.
   * @return `true` if #Azure::ETag has a value, `false` otherwise.
   */
  ...
  /**
   * @brief Compare with \p other #Azure::ETag for equality.
   * @param other Other #Azure::ETag to compare with.
   * @return `true` if #Azure::ETag instances are equal according to strong validation,
   * `false` otherwise.
   */
  bool operator==(const ETag& other) const { return Equals(*this, other, ETagComparison::Strong); }

  /**
   * @brief Compare with \p other #Azure::ETag for inequality.
   * @param other Other #Azure::ETag to compare with.
   * @return `true` if #Azure::ETag instances are not equal according to strong validation,
   * `false` otherwise.
   */
   ...
   * @return `true` if #Azure::ETag is a weak validator, `false` otherwise.
   */
  bool IsWeak() const
  {
