...
// The way it works is this: each library has its own AZURE_xxx_DLLEXPORT macro, which is used as
// prefix for public(*) static variables(**), this way: class Class { public:
//   AZURE_xxx_DLLEXPORT static bool IsSomething;
// }
// And also each cmake file adds a corresponding build definition:
// add_compile_build_definitions(AZURE_xxx_BEING_BUILT), so it IS defined at the moment the specific
// library (xxx) is being built, and is not defined at all other times. The AZURE_xxx_DLLEXPORT
// macro makes the static variable to be prefixed with dllexport attribute at a time when the (xxx)
// is being built, and all other code (other libraries, customer code) see it as dllimport
// attribute, when they include the header and consume the static variable from the (xxx) library.
// For that reason, each library should have its own AZURE_xxx_DLLEXPORT macro: when we build (yyy)
...
// libraries, do not reuse AZURE_xxx_DLLEXPORT, AZURE_xxx_BEING_BUILT, AZURE_xxx_DLL, or
// AZURE_xxx_BUILT_AS_DLL names.
//
// CMakeLists.txt (via az_vcpkg_export()) makes sure that during the SDK build on Windows when SDK
// is being built as a DLL, AZURE_xxx_DLL is defined. It is also being propagated to any code that
...
// (/*(at)AZURE_xxx_DLL_INSTALLED_AS_PACKAGE(at)*/ will be replaced with "/**/ + 1 /**/") if the SDK
// library was built as Windows DLL. CMakeLists.txt snippet to achieve all this is the following
// (***):
//
//   if(WIN32 AND BUILD_SHARED_LIBS)
//     add_compile_definitions(AZURE_xxx_BEING_BUILT)
//     target_compile_definitions(azure-xxx PUBLIC AZURE_xxx_DLL)
//
//     set(AZURE_xxx_DLL_INSTALLED_AS_PACKAGE "*/ + 1 /*")
//     configure_file(
...
//        @ONLY)
//    unset(AZURE_xxx_DLL_INSTALLED_AS_PACKAGE)
//   endif()
//
...
// handling AZURE_xxx_BUILT_AS_DLL correctly), there is always an option for th consumer to define
// AZURE_xxx_DLL manually.
// --
...
#if defined(AZURE_CORE_DLL) || (0 /*@AZURE_CORE_DLL_INSTALLED_AS_PACKAGE@*/)
#define AZURE_CORE_BUILT_AS_DLL 1
#else
#define AZURE_CORE_BUILT_AS_DLL 0
#endif

#if AZURE_CORE_BUILT_AS_DLL
#if defined(_MSC_VER)
#if defined(AZURE_CORE_BEING_BUILT)
#define AZURE_CORE_DLLEXPORT __declspec(dllexport)
#else // !defined(AZURE_CORE_BEING_BUILT)
#define AZURE_CORE_DLLEXPORT __declspec(dllimport)
#endif // AZURE_CORE_BEING_BUILT
#else // !defined(_MSC_VER)
#define AZURE_CORE_DLLEXPORT
#endif // _MSC_VER
#else // !AZURE_CORE_BUILT_AS_DLL
#define AZURE_CORE_DLLEXPORT
#endif // AZURE_CORE_BUILT_AS_DLL

#undef AZURE_CORE_BUILT_AS_DLL
