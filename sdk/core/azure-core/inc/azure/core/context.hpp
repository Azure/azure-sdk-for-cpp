  ...
  public:
    ...
    Context WithDeadline(time_point cancelWhen) const
    {
      return Context{std::make_shared<ContextSharedState>(
          m_contextSharedState, cancelWhen, std::string(), nullptr)};
    }
     ...
     */
    Context WithValue(const std::string& key, void* value) const
    {
      return Context{std::make_shared<ContextSharedState>(
          m_contextSharedState, (time_point::max)(), key, value)};
    }
     ...
     *
     * @return A value associated with the context found; null ptr if a specific value can't
     * be found.
     */
