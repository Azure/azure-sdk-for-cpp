...
namespace Azure { namespace Core { namespace IO {
  ...
  public:
    ...
    {
      AZURE_ASSERT(buffer || count == 0); // LCOV_EXCL_LINE
  ...
  public:
    ...
    {
      AZURE_ASSERT(data || length == 0); // LCOV_EXCL_LINE
    }
  ...
  namespace _internal {
    ...
    public:
      ...
      {
        AZURE_ASSERT(fileDescriptor >= 0 && offset >= 0 && length >= 0); // LCOV_EXCL_LINE
      }
      ...
      {
        AZURE_ASSERT(fileHandle && offset >= 0 && length >= 0); // LCOV_EXCL_LINE
      }
