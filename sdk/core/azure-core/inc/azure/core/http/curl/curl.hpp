...
// Cleaner runs every 5 min or every 1 second for unit tests
    ...
    struct CurlConnectionPool
    {
       ...
       */
      static std::map<std::string, std::list<std::unique_ptr<CurlConnection>>>
          s_connectionPoolIndex;
...
#ifdef TESTING_BUILD
      static bool IsCleanerRunning()
      {
        bool running = false;
        {
          std::lock_guard<std::mutex> lock(CurlConnectionPool::s_connectionPoolMutex);
          running = CurlConnectionPool::s_isCleanConnectionsRunning;
        }
        return running;
      }
#endif
    ...
    private:
       ...
       */
      uint8_t
          m_readBuffer[Details::c_DefaultLibcurlReaderSize]; // to work with libcurl custom read.
