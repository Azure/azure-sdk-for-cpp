...
TEST(Pipeline, createPipeline)
{
  ...
  EXPECT_NO_THROW(Azure::Core::Http::Internal::HttpPipeline pipeline(policies));
}
...
TEST(Pipeline, createEmptyPipeline)
{
  ...
  std::vector<std::unique_ptr<Azure::Core::Http::HttpPolicy>> policies;
  EXPECT_THROW(Azure::Core::Http::Internal::HttpPipeline pipeline(policies), std::invalid_argument);
}
...
TEST(Pipeline, clonePipeline)
{
  ...
  EXPECT_NO_THROW(Azure::Core::Http::Internal::HttpPipeline pipeline2(pipeline));
}

TEST(Pipeline, refrefPipeline)
{
  // Construct pipeline without exception
  EXPECT_NO_THROW(Azure::Core::Http::Internal::HttpPipeline pipeline(
