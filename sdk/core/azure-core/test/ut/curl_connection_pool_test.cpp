...
/***********************  Unique Tests for Libcurl   ********************************/
    ...
    TEST(CurlConnectionPool, forceConnectionClosed)
    {
      Azure::Core::Http::Request req(
          ...
          Azure::Core::Url(AzureSdkHttpbinServer::Status(101)));
      ...
      {
        // Check we can use the connection to retrieve headers in the response, but the connection is still flagged as shutdown.
        auto session
            = std::make_unique<Azure::Core::Http::CurlSession>(req, std::move(connection), options);
