...
void TracePC::PrintCoverage()
{
  ...
  for (size_t i = 1; i < GetNumPCs(); i++)
  {
    ...
    if (!EF->__sanitizer_get_module_and_offset_for_pc(
            reinterpret_cast<void*>(PCs[i]), ModulePathRaw, sizeof(ModulePathRaw), &OffsetRaw))
      continue;
...
void TracePC::DumpCoverage() { __sanitizer_dump_coverage(PCs, GetNumPCs()); }
...
extern "C"
{
  ...
  __attribute__((visibility("default"))) void __sanitizer_cov_trace_pc_guard_init(
      uint32_t* Start,
      uint32_t* Stop)
  {
