...
void Fuzzer::ShuffleCorpus(UnitVector* V)
{
  ...
  if (Options.PreferSmall)
    std::stable_sort(
        V->begin(), V->end(), [](const Unit& A, const Unit& B) { return A.size() < B.size(); });
}
...
void Fuzzer::TryDetectingAMemoryLeak(
    const uint8_t* Data,
    size_t Size,
...
void Fuzzer::MutateAndTestOne()
{
  ...
  for (int i = 0; i < Options.MutateDepth; i++)
  {
    ...
    {
      Corpus.AddToCorpus(
          {CurrentUnitData, CurrentUnitData + Size},
          NumFeatures,
          /*MayDeleteFile=*/true);
