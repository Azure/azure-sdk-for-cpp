...
public:
  ...
  void TraceSwitchCallback(
      uintptr_t PC,
      size_t ValSizeInBits,
      uint64_t Val,
      size_t NumCases,
      uint64_t* Cases);
...
int TraceState::TryToAddDesiredData(
    const uint8_t* PresentData,
...
void TraceState::TraceSwitchCallback(
    uintptr_t PC,
    size_t ValSizeInBits,
    uint64_t Val,
    size_t NumCases,
...
#define LLVM_FUZZER_DEFINES_SANITIZER_WEAK_HOOOKS 1
      ...
      int result)
  {
    ...
    TS->TraceMemcmpCallback(
        n, reinterpret_cast<const uint8_t*>(s1), reinterpret_cast<const uint8_t*>(s2));
  }
  ...
  void __sanitizer_weak_hook_memmem(
      void* called_pc,
      const void* s1,
      size_t len1,
      const void* s2,
