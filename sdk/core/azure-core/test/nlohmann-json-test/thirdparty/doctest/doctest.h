...
#define DOCTEST_FORBIT_EXPRESSION(rt, op) \
  ...
  { \
    static_assert( \
        deferred_false<R>::value, "Expression Too Complex Please Rewrite As Binary Comparison!"); \
    return *this; \
  }
  ...
  // DOCTEST_MSVC_SUPPRESS_WARNING(4805) // 'operation' : unsafe mix of type 'type' and type 'type'
  // in operation
...
#define DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(id, anon, ...) \
  DOCTEST_GLOBAL_NO_WARNINGS( \
      DOCTEST_CAT( \
          anon, DUMMY), /* NOLINT(cert-err58-cpp, fuchsia-statically-constructed-objects) */ \
    ...
    while (*str)
    {
      if (*wild == '*')
      {
        ...
        cp = str + 1;
          ...
          (caseSensitive ? (*wild == *str) : (tolower(*wild) == tolower(*str))) || (*wild == '?'))
      {
        wild++;
        str++;
      }
