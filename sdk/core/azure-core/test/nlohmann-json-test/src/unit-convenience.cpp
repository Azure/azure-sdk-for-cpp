...
struct alt_string_iter
{
  ...
  alt_string_iter(const char* cstr) : impl(cstr) {}

  void reserve(std::size_t s) { impl.reserve(s); }

  template <typename Iter> void append(Iter first, Iter last) { impl.append(first, last); }

  std::string::const_iterator begin() const noexcept { return impl.begin(); }

  std::string::const_iterator end() const noexcept { return impl.end(); }
...
struct alt_string_data
{
  ...
  alt_string_data(const char* cstr) : impl(cstr) {}

  void reserve(std::size_t s) { impl.reserve(s); }

  void append(const char* p, std::size_t s) { impl.append(p, s); }
...
TEST_CASE("convenience functions")
{
  ...
  {
    using Azure::Core::Json::_internal::detail::concat;
    ...
    SECTION("std::string")
    {
      std::string str1 = Azure::Core::Json::_internal::detail::concat(hello_iter, world, '!');
      std::string str2 = Azure::Core::Json::_internal::detail::concat(hello_data, world, '!');
      std::string str3 = Azure::Core::Json::_internal::detail::concat("Hello, ", world, '!');
    ...
    {
      alt_string_iter str
          = Azure::Core::Json::_internal::detail::concat<alt_string_iter>(hello_iter, world, '!');
    ...
    {
      alt_string_data str
          = Azure::Core::Json::_internal::detail::concat<alt_string_data>(hello_data, world, '!');
