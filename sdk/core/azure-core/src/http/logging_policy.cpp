...
std::string TruncateIfLengthy(std::string const& s)
{
  ...
  return s.substr(0, BeginLength) + Ellipsis + s.substr(0, length - EndLength, EndLength);
}
    ...
    NextHttpPolicy nextHttpPolicy) const
{
  ...
  Logging::Details::Write(
      LogClassification::Response, GetRequestLogMessage(request, response.get(), end - start));
