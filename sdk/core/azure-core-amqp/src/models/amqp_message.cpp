...
namespace Azure { namespace Core { namespace Amqp { namespace Models {
            ...
            if (amqpvalue_get_type(describedProperties.get()) == AMQP_TYPE_DESCRIBED)
            {
              ...
              if (amqpvalue_get_ulong(describedType, &describedTypeValue))
              {
                throw std::runtime_error(
                    "Could not retrieve application properties described type.");
              }
              ...
              {
                throw std::runtime_error(
                    "Application Properties are not the corect described type.");
              }
    ...
    {
      //      size_t serializedSize = AmqpMessage::GetSerializedSize(message);
      ...
      if (!message.ApplicationProperties.empty())
      {
        ...
        AmqpValue propertiesValue{
            amqpvalue_create_application_properties(AmqpValue{appProperties})};
        auto serializedApplicationProperties = AmqpValue::Serialize(propertiesValue);
        rv.insert(
            rv.end(),
            serializedApplicationProperties.begin(),
            serializedApplicationProperties.end());
      }
      ...
      switch (message.BodyType)
      {
        ...
        case MessageBodyType::Data:
          ...
          {
            AmqpDescribed describedBody(
                static_cast<std::uint64_t>(AmqpDescriptors::DataBinary), val);
            auto serializedBodyValue = AmqpValue::Serialize(describedBody);
            rv.insert(rv.end(), serializedBodyValue.begin(), serializedBodyValue.end());
          }
          break;
    ...
    std::vector<Amqp::_detail::AmqpDescriptors> expectedMessageFields{
              ...
              static_cast<AmqpDescriptors>(static_cast<uint64_t>(describedType.GetDescriptor())))
          {
            ...
            case AmqpDescriptors::Properties: {
              ...
              m_decodedValue.Properties
                  = _internal::MessagePropertiesFactory::FromUamqp(properties);
              break;
            }
