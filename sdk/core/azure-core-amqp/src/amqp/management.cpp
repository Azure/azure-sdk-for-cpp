...
#include "azure/core/amqp/private/management_impl.hpp"
    ...
    namespace _detail {
        ...
        if (result)
        {
          switch (std::get<0>(*result))
          {
            ...
            case AMQP_MANAGEMENT_OPEN_ERROR:
              return _internal::ManagementOpenResult::Error;
            case AMQP_MANAGEMENT_OPEN_CANCELLED:
              return _internal::ManagementOpenResult::Cancelled;
            default:
              throw std::runtime_error("Unknown management open result.");
          }
        }
          ...
          Azure::Core::Context context)
      {
        if (!amqp_management_execute_operation_async(
                ...
                this))
        {
          throw std::runtime_error("Could not execute operation.");
        }

        auto result
            = m_messageQueue.WaitForPolledResult(context, *m_session->GetConnectionToPoll());
        if (result)
        {
          ...
          MESSAGE_HANDLE message)
      {
        ...
        switch (result)
        {
          ...
          case AMQP_MANAGEMENT_EXECUTE_OPERATION_INSTANCE_CLOSED:
            ...
            break;
          default:
            throw std::runtime_error("Unknown management status.");
        }
      }
