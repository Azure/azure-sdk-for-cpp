 ...
 *
 */
class AmqpMap final : public _detail::AmqpCollectionBase<std::map<AmqpValue, AmqpValue>, AmqpMap> {
...
/** @brief An AMQP symbol value. An AMQP Symbol is a string valued from a constrained domain, whose
 * content is sequence of ASCII encoded characters.
 *
 * Defined in [AMQP Core Types
...
public:
   ...
   * @returns true if the two symbols are equal, false otherwise.
   */
  bool operator==(const decltype(m_value)::value_type* const that) const { return m_value == that; }
};
