...
- A uAMQP link error now wakes every send that waits on that link. The sender tracked one send at a time, so a second send to the same partition replaced the first. The first send then waited for its full deadline. `ProducerClient::Send` lets sends to one partition run together. The sender now holds each send that waits and removes only the send that finished. [[#7254]](https://github.com/Azure/azure-sdk-for-cpp/issues/7254)
