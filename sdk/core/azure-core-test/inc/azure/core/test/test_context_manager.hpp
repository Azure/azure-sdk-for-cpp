...
#include <gtest/gtest.h>
    ...
    namespace Azure { namespace Core { namespace Test {
      /**
       * @brief The test context is used to hold information about a test, such as the recording
       * path, running test mode, name, etc.
       *
       */
      class TestContextManager {
      public:
        /**
         * @brief The path where the asset.json for the current test exists, will be passed as part
         * of the playback request to the test-proxy via the manager.
         *
         */
        std::string AssetsPath;
        /**
         ...
         * @brief Whenever the test must be ran on live mode only.
         *
         * @remark This configuration allow tests to ignore the recording or playback setting and
         * run without it and amount other tests which are using the recording and playback.
         *
         */
        bool LiveOnly = false;
      ...
      private:
        ...
        void SetRunFlags(std::string const& testName)
        {
          ...
          // Naming a test with a suffix `RECORDEDONLY` will set it up to be only PAYBACK/RECORD
          // mode supported. it won't be running in live mode.
