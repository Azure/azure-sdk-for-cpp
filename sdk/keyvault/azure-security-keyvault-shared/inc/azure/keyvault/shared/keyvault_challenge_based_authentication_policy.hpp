// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License.

/**
 * @file
 * @brief Key Vault Challenge-Based Authentication Policy.
 *
 */

#pragma once

// This header file is left for compatibility purposes.
// The name of this header is long, and may cause build error on Windows systems on some installations,
// with long path and maximum path length of 260 characters.

#include <azure/keyvault/shared/keyvault_challenge_based_auth.hpp>
