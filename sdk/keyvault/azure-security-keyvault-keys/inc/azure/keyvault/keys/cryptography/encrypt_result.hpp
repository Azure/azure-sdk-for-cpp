...
namespace Azure {
  ...
  {
    /**
     * @brief Gets the key identifier of the #KeyVaultKey used to encrypt. This must be
     * stored alongside the #Ciphertext as the same key must be used to decrypt it.
     *
     */
    std::string KeyId;
    ...
    EncryptResult(EncryptionAlgorithm const& algorithm) : Algorithm(algorithm) {}
  };
