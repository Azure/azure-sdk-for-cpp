 ...
 * `AZ_SECURITY_KEYVAULT_SECRETS_WITH_RTTI`. When the symbol is not exported, RTTI is disabled.
 *
...
/**
 * @def AZ_SECURITY_KEYVAULT_SECRETS_WITH_RTTI
 ...
 * header requires to resolve `AZ_SECURITY_KEYVAULT_SECRETS_WITH_RTTI` which will become ` + 1 `
 * when the SDK was built with RTTI and `false` otherwise.
 */

#if AZ_BUILD_RTTI || (0 /*@AZ_SECURITY_KEYVAULT_SECRETS_WITH_RTTI@*/)
#define AZ_SECURITY_KEYVAULT_SECRETS_WITH_RTTI 1
#endif
