  ...
  std::string SharedKeyPolicy::GetSignature(const Core::Http::Request& request) const
  {
    ...
    for (std::string headerName :
         {"Content-Encoding",
          ...
          "Range"})
    {
      auto ite = headers.find(Azure::Core::Internal::ToLower(headerName));
      if (ite != headers.end())
      {
    ...
    for (auto ite = headers.lower_bound(prefix);
         ...
         ++ite)
    {
      std::string key = Azure::Core::Internal::ToLower(ite->first);
    ...
    {
      std::string key = Azure::Core::Internal::ToLower(query.first);
