...
namespace Azure { namespace Identity { namespace Test {
  ...
  private:
    ...
    std::string GetAzCommand(std::string const& resource, std::string const& tenantId)
        const override
    {
  ...
  public:
    ...
    decltype(m_cliProcessTimeout) const& GetCliProcessTimeout() const
    {
      return m_cliProcessTimeout;
    }
...
#else // UWP
      // The credential should throw during GetToken() and not during construction, because it
      // allows customers to put it into ChainedTokenCredential and successfully use it there
      // without writing ifdefs for UWP. It is not too late to throw - for example, if Azure CLI is
      // not installed, then the credential will also find out during GetToken() and not during
      // construction (if we had to find out during the construction, we'd have to fire up some 'az'
      // command in constructor; again, that would also make it hard to put the credential into
