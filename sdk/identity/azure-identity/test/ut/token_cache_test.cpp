...
namespace Azure { namespace Identity { namespace Test {
  ...
  TEST(TokenCache, TwoThreadsAttemptToUpdateTheSameToken)
  {
...

    {
      ...
      auto const token = tokenCache.GetToken("A", {}, 2min, [=]() {
        EXPECT_FALSE(
            "getNewToken does not get invoked when the fresh value was inserted just before "
  ...
  TEST(TokenCache, ExpiredCleanup)
  {
    ...
    // Re-add items 2 and 3. Adding them should not trigger cleanup. After adding, cache should get
    // to 55 items (with numbers from 2 to 56, and number 1 missing).
    for (auto i = 2; i <= 3; ++i)
    {
      auto const n = std::to_string(i);
      static_cast<void>(tokenCache.GetToken(n, {}, 2min, [=]() {
    ...
    // Now let's lock some of the items for reading, and some for writing. Cleanup should not block
    // on token release, but will simply move on, without doing anything to the ones that were
    // locked. Out of 4 locked, two are expired, so they should get cleared under normal
    // circumstances, but this time they will remain in the cache.
        ...
        std::string clientSecret;
        {
          ...
          if (clientSecretStartPos != std::string::npos)
          {
            ...
            clientSecret = (clientSecretEndPos == std::string::npos)
                ...
                : reqBodyStr.substr(
                    clientSecretStartPos, clientSecretEndPos - clientSecretStartPos);
          }
        }
