...
TEST(DefaultAzureCredential, LogMessages)
{
  ...
  CredentialTestHelper::SimulateTokenRequest(
      [&](auto transport) {
        ...
        EXPECT_EQ(log.size(), LogMsgVec::size_type(12));
        ...
        EXPECT_EQ(log[3].second, "Identity: ClientAssertionCredential was created successfully.");

        EXPECT_EQ(log[4].first, Logger::Level::Informational);
        EXPECT_EQ(log[4].second, "Identity: WorkloadIdentityCredential was created successfully.");
        ...
        EXPECT_EQ(log[9].first, Logger::Level::Verbose);
        EXPECT_EQ(
            log[9].second,
            "Identity: ManagedIdentityCredential: Environment is not set up for the credential "
        ...
        EXPECT_EQ(log[5].first, Logger::Level::Informational);
        EXPECT_EQ(
            log[5].second,
            "Identity: AzureCliCredential created."
            "\nSuccessful creation does not guarantee further successful token retrieval.");

        EXPECT_EQ(log[11].first, Logger::Level::Informational);
        EXPECT_EQ(
            log[11].second,
