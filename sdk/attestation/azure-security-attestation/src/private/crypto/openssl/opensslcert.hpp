...
namespace Azure { namespace Security { namespace Attestation { namespace _private {
  namespace Cryptography {
    ...
    public:
      ...
      static std::string ToHexString(_In_ std::vector<uint8_t> const& buffer)
      {
      ...
      std::string GetThumbprint() const override
      {
        ...
        {
          throw _details::OpenSSLException("i2d_x509");
        }
      ...
      std::string GetAlgorithm() const override
      {
        ...
        if (X509_PUBKEY_get0_param(&asn1Algorithm, &publicKey, &publicKeyLen, &algorithm, pubkey)
            != 1)
        {
          throw _details::OpenSSLException("X509_PUBKEY_get0_param");
        }
