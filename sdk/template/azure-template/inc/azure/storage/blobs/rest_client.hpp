...
namespace Models {
/**
 * @brief The algorithm used to produce the encryption key hash. Currently, the
 * only accepted value is "AES256". Must be provided if the x-ms-encryption-key
 * header is provided.
 */
class EncryptionAlgorithmType final {
public:
  EncryptionAlgorithmType() = default;
  explicit EncryptionAlgorithmType(std::string value)
  ...
  AZ_TEMPLATE_DLLEXPORT const static EncryptionAlgorithmType Aes256;

private:
  std::string m_value;
};
/**
 * @brief Extensible enum used to specify how the service should look for a
 * block ID.
 */
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static BlockType Committed;
  AZ_TEMPLATE_DLLEXPORT const static BlockType Uncommitted;
  AZ_TEMPLATE_DLLEXPORT const static BlockType Latest;

private:
  std::string m_value;
};
/**
 * @brief The retention policy which determines how long the associated data
 * should persist.
 */
...
/**
 * @brief A summary of request statistics grouped by API in hour or minute
...
/**
 * @brief CORS is an HTTP feature that enables a web application running under
 * one domain to access resources in another domain. Web browsers implement a
...
struct CorsRule final {
   ...
   * originates. Note that the origin must be an exact case-sensitive match with
   * the origin that the user age sends to the service. You can also use the
   * wildcard character '*' to allow all origin domains to make requests via
   * CORS.
   */
 ...
 * #Azure::Storage::Blobs::ServiceClient::SetProperties.
 */
...
public:
  ...
  explicit GeoReplicationStatus(std::string value)
      : m_value(std::move(value)) {}
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static GeoReplicationStatus Live;
  AZ_TEMPLATE_DLLEXPORT const static GeoReplicationStatus Bootstrap;
  AZ_TEMPLATE_DLLEXPORT const static GeoReplicationStatus Unavailable;
...
struct GeoReplication final {
   ...
   * value are guaranteed to be available for read operations at the secondary.
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static LeaseStatus Locked;
  AZ_TEMPLATE_DLLEXPORT const static LeaseStatus Unlocked;
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static LeaseState Available;
  AZ_TEMPLATE_DLLEXPORT const static LeaseState Leased;
  AZ_TEMPLATE_DLLEXPORT const static LeaseState Expired;
  AZ_TEMPLATE_DLLEXPORT const static LeaseState Breaking;
  AZ_TEMPLATE_DLLEXPORT const static LeaseState Broken;

private:
  std::string m_value;
};
/**
 * @brief When a blob is leased, specifies whether the lease is of infinite or
 * fixed duration.
 */
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static LeaseDurationType Infinite;
  AZ_TEMPLATE_DLLEXPORT const static LeaseDurationType Fixed;

private:
  std::string m_value;
};
/**
 * @brief Specifies whether data in the container may be accessed publicly and
 * the level of access.
 */
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static PublicAccessType BlobContainer;
  AZ_TEMPLATE_DLLEXPORT const static PublicAccessType Blob;
  AZ_TEMPLATE_DLLEXPORT const static PublicAccessType None;
...
struct BlobContainerItemDetails final {
  ...
  /**
   * Data and time at which this container was deleted. Only valid when this
   * container was deleted.
   */
  Nullable<DateTime> DeletedOn;
  /**
   * Remaining days before this container will be permanantely deleted. Only
   * valid when this container was deleted.
   */
...
/**
 * @brief Include this parameter to specify that the container's metadata be
...
operator|(ListBlobContainersIncludeFlags lhs,
          ListBlobContainersIncludeFlags rhs) {
  using type = std::underlying_type_t<ListBlobContainersIncludeFlags>;
  return static_cast<ListBlobContainersIncludeFlags>(static_cast<type>(lhs) |
...
public:
  ...
  bool operator==(const SkuName &other) const {
    return m_value == other.m_value;
  }
  bool operator!=(const SkuName &other) const { return !(*this == other); }
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static SkuName StandardLrs;
  AZ_TEMPLATE_DLLEXPORT const static SkuName StandardGrs;
  AZ_TEMPLATE_DLLEXPORT const static SkuName StandardRagrs;
  AZ_TEMPLATE_DLLEXPORT const static SkuName StandardZrs;
  AZ_TEMPLATE_DLLEXPORT const static SkuName PremiumLrs;
  AZ_TEMPLATE_DLLEXPORT const static SkuName PremiumZrs;
  AZ_TEMPLATE_DLLEXPORT const static SkuName StandardGzrs;
  AZ_TEMPLATE_DLLEXPORT const static SkuName StandardRagzrs;
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static AccountKind Storage;
  AZ_TEMPLATE_DLLEXPORT const static AccountKind BlobStorage;
  AZ_TEMPLATE_DLLEXPORT const static AccountKind StorageV2;
  AZ_TEMPLATE_DLLEXPORT const static AccountKind FileStorage;
  AZ_TEMPLATE_DLLEXPORT const static AccountKind BlockBlobStorage;
...
/**
 * @brief Response type for
 * #Azure::Storage::Blobs::ServiceClient::GetAccountInfo.
 */
struct AccountInfo final {
  ...
  /**
   * Version 2019-07-07 and newer. Indicates if the account has a hierarchical
   * namespace enabled.
   */
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static CopyStatus Pending;
  AZ_TEMPLATE_DLLEXPORT const static CopyStatus Success;
  AZ_TEMPLATE_DLLEXPORT const static CopyStatus Aborted;
  AZ_TEMPLATE_DLLEXPORT const static CopyStatus Failed;
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P1;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P2;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P3;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P4;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P6;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P10;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P15;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P20;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P30;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P40;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P50;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P60;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P70;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier P80;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier Hot;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier Cool;
  AZ_TEMPLATE_DLLEXPORT const static AccessTier Archive;
 ...
 * rehydrated and is not complete then this value indicates that rehydrate is
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static ArchiveStatus RehydratePendingToHot;
  AZ_TEMPLATE_DLLEXPORT const static ArchiveStatus RehydratePendingToCool;

private:
  std::string m_value;
};
/**
 * @brief Optional: Indicates the priority with which to rehydrate an archived
 * blob.
 */
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static RehydratePriority High;
  AZ_TEMPLATE_DLLEXPORT const static RehydratePriority Standard;
...
struct BlobHttpHeaders final {
  ...
  /**
   * Conveys additional information about how to process the resource payload,
...
/**
 * @brief The replication status of blob with the given policy and rule
 * identifiers.
 */
class ObjectReplicationStatus final {
public:
  ObjectReplicationStatus() = default;
  explicit ObjectReplicationStatus(std::string value)
      : m_value(std::move(value)) {}
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static ObjectReplicationStatus Complete;
  AZ_TEMPLATE_DLLEXPORT const static ObjectReplicationStatus Failed;

private:
  std::string m_value;
};
/**
 * @brief Contains the object replication rule ID and replication status of a
 * blob.
 */
 ...
 * #ObjectReplicationRule s. This is used when retrieving the object replication
 * properties on the source blob.
 */
struct ObjectReplicationPolicy final {
  ...
  std::string PolicyId;
  /**
   * The Rule IDs and respective replication status that are under the policy
   * ID.
   */
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static BlobImmutabilityPolicyMode Unlocked;
  AZ_TEMPLATE_DLLEXPORT const static BlobImmutabilityPolicyMode Locked;
...
struct BlobImmutabilityPolicy final {
  /**
   * The date until which the blob can be protected from being modified or
   * deleted.
   */
  DateTime ExpiresOn;
  /**
...
struct BlobItemDetails final {
  ...
  /**
   * URL up to 2 KB in length that specifies the source blob or file used in the
   * last attempted Copy Blob operation where this blob was the destination
   ...
   * metadata are completely encrypted using the specified algorithm. Otherwise,
   * the value is set to false (when the blob is unencrypted, or if only parts
   * of the blob/application metadata are encrypted).
   */
  ...
  /**
   * Data and time at which this blob was deleted. Only valid when this blob was
   * deleted.
   */
  Nullable<DateTime> DeletedOn;
  /**
   * Remaining days before this blob will be permanantely deleted. Only valid
   * when this blob was deleted.
   */
  Nullable<int32_t> RemainingRetentionDays;
  /**
   * The tier of page blob on a premium storage account or tier of block blob on
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static BlobType BlockBlob;
  AZ_TEMPLATE_DLLEXPORT const static BlobType PageBlob;
  AZ_TEMPLATE_DLLEXPORT const static BlobType AppendBlob;
...
/**
 * @brief Include this parameter to specify one or more datasets to include in
 * the response.
 */
...
inline ListBlobsIncludeFlags operator|(ListBlobsIncludeFlags lhs,
                                       ListBlobsIncludeFlags rhs) {
  using type = std::underlying_type_t<ListBlobsIncludeFlags>;
  return static_cast<ListBlobsIncludeFlags>(static_cast<type>(lhs) |
                                            static_cast<type>(rhs));
}
inline ListBlobsIncludeFlags &operator|=(ListBlobsIncludeFlags &lhs,
                                         ListBlobsIncludeFlags rhs) {
...
struct DownloadBlobDetails final {
  ...
  /**
   * If the blob has been sealed. This value is null for block blobs or page
   * blobs.
   */
  Nullable<bool> IsSealed;
  /**
  ...
  /**
   * True if the blob data and metadata are completely encrypted using the
   * specified algorithm. Otherwise, the value is set to false (when the blob is
   * unencrypted, or if only parts of the blob/application metadata are
   * encrypted).
   */
  bool IsServerEncrypted = bool();
  /**
   * SHA-256 hash of the encryption key used to encrypt the blob data and
   * metadata.
   */
  Nullable<std::vector<uint8_t>> EncryptionKeySha256;
  /**
  ...
  Nullable<std::string> EncryptionScope;
  /**
   * Only valid when Object Replication is enabled and current blob is the
   * destination.
   */
  ...
  Nullable<DateTime> CopyCompletedOn;
  /**
   * A string value returned by the service that uniquely identifies the blob
   * version.
   */
  Nullable<std::string> VersionId;
  /**
...
struct BlobProperties final {
  ...
  Nullable<int64_t> SequenceNumber;
  /**
   * The number of committed blocks present in the blob. This header is returned
   * only for append blobs.
   */
  Nullable<int32_t> CommittedBlockCount;
  /**
   * The value of this header is set to true if the blob data and application
   ...
   * and application metadata.  Note that the absence of this header implies use
   * of the default account encryption scope.
   */
  Nullable<std::string> EncryptionScope;
  /**
   * The tier of page blob on a premium storage account or tier of block blob on
   * blob storage LRS accounts. For a list of allowed premium page blob tiers,
   * see
  ...
  Nullable<Models::AccessTier> AccessTier;
  /**
   * For page blobs on a premium storage account only. If the access tier is not
   * explicitly set on the blob, the tier is inferred based on its content
   ...
   * For blob storage LRS accounts, valid values are
   * rehydrate-pending-to-hot/rehydrate-pending-to-cool. If the blob is being
   * rehydrated and is not complete then this header is returned indicating that
   ...
   * The value of this header indicates the blob version, and may be used in
   * subsequent requests to access this version of the blob.
   */
  Nullable<std::string> VersionId;
  /**
...
/**
 * @brief Required if the blob has associated snapshots. Specify one of the
...
public:
  ...
  explicit DeleteSnapshotsOption(std::string value)
      : m_value(std::move(value)) {}
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static DeleteSnapshotsOption
      IncludeSnapshots;
  AZ_TEMPLATE_DLLEXPORT const static DeleteSnapshotsOption OnlySnapshots;
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static ScheduleBlobExpiryOriginType
      NeverExpire;
  AZ_TEMPLATE_DLLEXPORT const static ScheduleBlobExpiryOriginType
      RelativeToCreation;
  AZ_TEMPLATE_DLLEXPORT const static ScheduleBlobExpiryOriginType
      RelativeToNow;
  AZ_TEMPLATE_DLLEXPORT const static ScheduleBlobExpiryOriginType Absolute;
...
/**
 * @brief Response type for
 * #Azure::Storage::Blobs::BlobClient::SetImmutabilityPolicy.
 */
...
/**
 * @brief Response type for
 * #Azure::Storage::Blobs::BlobClient::DeleteImmutabilityPolicy.
 */
...
struct SetBlobMetadataResult final {
  /**
   * The field is deprecated and is always null. Use GetProperties() instead to
   * check sequence number for a page blob.
   */
  Nullable<int64_t> SequenceNumber;
  /**
   * The ETag contains a value that you can use to perform operations
...
struct CreateBlobSnapshotResult final {
  /**
   * The field is deprecated and is always null. Use GetProperties() instead to
   * get SHA256 of the encryption key.
   */
  Nullable<std::vector<uint8_t>> EncryptionKeySha256;
  /**
   * The field is deprecated and is always null. Use GetProperties() instead to
   * check the encryption scope.
   */
  Nullable<std::string> EncryptionScope;
  /**
   * Uniquely identifies the snapshot and indicates the snapshot version. It may
   * be used in subsequent requests to access the snapshot.
   */
  std::string Snapshot;
  /**
   * The ETag contains a value that you can use to perform operations
...
struct CopyBlobFromUriResult final {
   ...
   * integrity of the copied content. This header is only returned if the source
   * content MD5 was specified.
   */
  Nullable<ContentHash> TransactionalContentHash;
};
/**
...
/**
 * @brief Required if the x-ms-blob-sequence-number header is set for the
 * request. This property applies to page blobs only. This property indicates
 * how the service should modify the blob's sequence number.
 */
class SequenceNumberAction final {
public:
  SequenceNumberAction() = default;
  explicit SequenceNumberAction(std::string value)
      : m_value(std::move(value)) {}
  bool operator==(const SequenceNumberAction &other) const {
    return m_value == other.m_value;
  }
  bool operator!=(const SequenceNumberAction &other) const {
    return !(*this == other);
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static SequenceNumberAction Max;
  AZ_TEMPLATE_DLLEXPORT const static SequenceNumberAction Update;
  AZ_TEMPLATE_DLLEXPORT const static SequenceNumberAction Increment;
...
/**
 * @brief Response type for
 * #Azure::Storage::Blobs::PageBlobClient::UpdateSequenceNumber.
 */
...
struct AppendBlockResult final {
  ...
  Nullable<ContentHash> TransactionalContentHash;
  /**
   * This response header is returned only for append operations. It returns the
   * offset at which the block was committed, in bytes.
   */
...
/**
 * @brief Response type for
 * #Azure::Storage::Blobs::BlockBlobClient::UploadFromUri.
 */
...
/**
 * @brief Represents a single block in a block blob.  It describes the block's
 * ID and size.
 */
...
/**
 * @brief Specifies whether to return the list of committed blocks, the list of
...
public:
  ...
  const std::string &ToString() const { return m_value; }
  AZ_TEMPLATE_DLLEXPORT const static BlockListType Committed;
  AZ_TEMPLATE_DLLEXPORT const static BlockListType Uncommitted;
  AZ_TEMPLATE_DLLEXPORT const static BlockListType All;
...
public:
  ...
  static Response<Models::_detail::ListBlobContainersResult>
  ListBlobContainers(Core::Http::_internal::HttpPipeline &pipeline,
                     const Core::Url &url,
  ...
  static Response<Models::UserDelegationKey>
  GetUserDelegationKey(Core::Http::_internal::HttpPipeline &pipeline,
                       const Core::Url &url,
                       const GetServiceUserDelegationKeyOptions &options,
                       const Core::Context &context);
  struct GetServiceAccountInfoOptions final {};
  static Response<Models::AccountInfo>
  GetAccountInfo(Core::Http::_internal::HttpPipeline &pipeline,
                 const Core::Url &url,
  ...
  static Response<Models::_detail::FindBlobsByTagsResult>
  FindBlobsByTags(Core::Http::_internal::HttpPipeline &pipeline,
                  const Core::Url &url,
...
public:
  ...
  static Response<Models::CreateBlobContainerResult>
  Create(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
         const CreateBlobContainerOptions &options,
         const Core::Context &context);
  struct GetBlobContainerPropertiesOptions final {
    Nullable<std::string> LeaseId;
  };
  static Response<Models::BlobContainerProperties>
  GetProperties(Core::Http::_internal::HttpPipeline &pipeline,
                const Core::Url &url,
  ...
  static Response<Models::DeleteBlobContainerResult>
  Delete(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
  ...
  static Response<Models::SetBlobContainerMetadataResult>
  SetMetadata(Core::Http::_internal::HttpPipeline &pipeline,
              const Core::Url &url,
  ...
  static Response<Models::BlobContainerAccessPolicy>
  GetAccessPolicy(Core::Http::_internal::HttpPipeline &pipeline,
                  const Core::Url &url,
  ...
  static Response<Models::SetBlobContainerAccessPolicyResult>
  SetAccessPolicy(Core::Http::_internal::HttpPipeline &pipeline,
                  const Core::Url &url,
  ...
  static Response<Models::_detail::UndeleteBlobContainerResult>
  Undelete(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
  ...
  static Response<Models::_detail::RenameBlobContainerResult>
  Rename(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
  ...
  static Response<Models::_detail::AcquireBlobContainerLeaseResult>
  AcquireLease(Core::Http::_internal::HttpPipeline &pipeline,
               const Core::Url &url,
  ...
  static Response<Models::_detail::ReleaseBlobContainerLeaseResult>
  ReleaseLease(Core::Http::_internal::HttpPipeline &pipeline,
               const Core::Url &url,
  ...
  static Response<Models::_detail::RenewBlobContainerLeaseResult>
  RenewLease(Core::Http::_internal::HttpPipeline &pipeline,
             const Core::Url &url,
  ...
  static Response<Models::_detail::BreakBlobContainerLeaseResult>
  BreakLease(Core::Http::_internal::HttpPipeline &pipeline,
             const Core::Url &url,
  ...
  static Response<Models::_detail::ChangeBlobContainerLeaseResult>
  ChangeLease(Core::Http::_internal::HttpPipeline &pipeline,
              const Core::Url &url,
  ...
  static Response<Models::_detail::ListBlobsResult>
  ListBlobs(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
  ...
  static Response<Models::_detail::ListBlobsByHierarchyResult>
  ListBlobsByHierarchy(Core::Http::_internal::HttpPipeline &pipeline,
                       const Core::Url &url,
...
public:
  ...
  static Response<Models::DownloadBlobResult>
  Download(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
           const DownloadBlobOptions &options, const Core::Context &context);
  ...
  static Response<Models::BlobProperties>
  GetProperties(Core::Http::_internal::HttpPipeline &pipeline,
                const Core::Url &url, const GetBlobPropertiesOptions &options,
                const Core::Context &context);
  ...
  static Response<Models::DeleteBlobResult>
  Delete(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
         const DeleteBlobOptions &options, const Core::Context &context);
  struct UndeleteBlobOptions final {};
  static Response<Models::UndeleteBlobResult>
  Undelete(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
           const UndeleteBlobOptions &options, const Core::Context &context);
  struct SetBlobExpiryOptions final {
    Models::ScheduleBlobExpiryOriginType ExpiryOptions;
    Nullable<std::string> ExpiresOn;
  };
  static Response<Models::SetBlobExpiryResult>
  SetExpiry(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
            const SetBlobExpiryOptions &options, const Core::Context &context);
  ...
  static Response<Models::SetBlobHttpHeadersResult>
  SetHttpHeaders(Core::Http::_internal::HttpPipeline &pipeline,
                 const Core::Url &url, const SetBlobHttpHeadersOptions &options,
                 const Core::Context &context);
  ...
  static Response<Models::SetBlobImmutabilityPolicyResult>
  SetImmutabilityPolicy(Core::Http::_internal::HttpPipeline &pipeline,
                        const Core::Url &url,
                        const SetBlobImmutabilityPolicyOptions &options,
                        const Core::Context &context);
  struct DeleteBlobImmutabilityPolicyOptions final {};
  static Response<Models::DeleteBlobImmutabilityPolicyResult>
  DeleteImmutabilityPolicy(Core::Http::_internal::HttpPipeline &pipeline,
                           const Core::Url &url,
  ...
  static Response<Models::SetBlobLegalHoldResult>
  SetLegalHold(Core::Http::_internal::HttpPipeline &pipeline,
               const Core::Url &url, const SetBlobLegalHoldOptions &options,
               const Core::Context &context);
  ...
  static Response<Models::SetBlobMetadataResult>
  SetMetadata(Core::Http::_internal::HttpPipeline &pipeline,
              const Core::Url &url, const SetBlobMetadataOptions &options,
              const Core::Context &context);
  ...
  static Response<Models::_detail::AcquireBlobLeaseResult>
  AcquireLease(Core::Http::_internal::HttpPipeline &pipeline,
               const Core::Url &url, const AcquireBlobLeaseOptions &options,
               const Core::Context &context);
  ...
  static Response<Models::_detail::ReleaseBlobLeaseResult>
  ReleaseLease(Core::Http::_internal::HttpPipeline &pipeline,
               const Core::Url &url, const ReleaseBlobLeaseOptions &options,
               const Core::Context &context);
  ...
  static Response<Models::_detail::RenewBlobLeaseResult>
  RenewLease(Core::Http::_internal::HttpPipeline &pipeline,
             const Core::Url &url, const RenewBlobLeaseOptions &options,
             const Core::Context &context);
  ...
  static Response<Models::_detail::ChangeBlobLeaseResult>
  ChangeLease(Core::Http::_internal::HttpPipeline &pipeline,
              const Core::Url &url, const ChangeBlobLeaseOptions &options,
              const Core::Context &context);
  ...
  static Response<Models::_detail::BreakBlobLeaseResult>
  BreakLease(Core::Http::_internal::HttpPipeline &pipeline,
             const Core::Url &url, const BreakBlobLeaseOptions &options,
             const Core::Context &context);
  ...
  static Response<Models::CreateBlobSnapshotResult>
  CreateSnapshot(Core::Http::_internal::HttpPipeline &pipeline,
                 const Core::Url &url, const CreateBlobSnapshotOptions &options,
                 const Core::Context &context);
  ...
  static Response<Models::_detail::StartBlobCopyFromUriResult> StartCopyFromUri(
      Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
      const StartBlobCopyFromUriOptions &options, const Core::Context &context);
  ...
  static Response<Models::CopyBlobFromUriResult>
  CopyFromUri(Core::Http::_internal::HttpPipeline &pipeline,
              const Core::Url &url, const CopyBlobFromUriOptions &options,
              const Core::Context &context);
  ...
  static Response<Models::SetBlobAccessTierResult>
  SetTier(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
          const SetBlobTierOptions &options, const Core::Context &context);
  ...
  static Response<std::map<std::string, std::string>>
  GetTags(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
          const GetBlobTagsOptions &options, const Core::Context &context);
  ...
  static Response<Models::SetBlobTagsResult>
  SetTags(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
          const SetBlobTagsOptions &options, const Core::Context &context);
};
...
public:
  ...
  static Response<Models::CreatePageBlobResult>
  Create(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
  ...
  static Response<Models::UploadPagesResult>
  UploadPages(Core::Http::_internal::HttpPipeline &pipeline,
  ...
  static Response<Models::ClearPagesResult>
  ClearPages(Core::Http::_internal::HttpPipeline &pipeline,
             const Core::Url &url, const ClearPageBlobPagesOptions &options,
             const Core::Context &context);
  ...
  static Response<Models::UploadPagesFromUriResult>
  UploadPagesFromUri(Core::Http::_internal::HttpPipeline &pipeline,
                     const Core::Url &url,
  ...
  static Response<Models::_detail::GetPageRangesResult>
  GetPageRanges(Core::Http::_internal::HttpPipeline &pipeline,
                const Core::Url &url,
  ...
  static Response<Models::_detail::GetPageRangesDiffResult>
  GetPageRangesDiff(Core::Http::_internal::HttpPipeline &pipeline,
                    const Core::Url &url,
  ...
  static Response<Models::ResizePageBlobResult>
  Resize(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
  ...
  static Response<Models::UpdateSequenceNumberResult>
  UpdateSequenceNumber(Core::Http::_internal::HttpPipeline &pipeline,
                       const Core::Url &url,
  ...
  static Response<Models::_detail::StartBlobCopyIncrementalResult>
  StartCopyIncremental(Core::Http::_internal::HttpPipeline &pipeline,
                       const Core::Url &url,
...
public:
  ...
  static Response<Models::CreateAppendBlobResult>
  Create(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
  ...
  static Response<Models::AppendBlockResult>
  AppendBlock(Core::Http::_internal::HttpPipeline &pipeline,
              const Core::Url &url, Core::IO::BodyStream &requestBody,
  ...
  static Response<Models::AppendBlockFromUriResult>
  AppendBlockFromUri(Core::Http::_internal::HttpPipeline &pipeline,
                     const Core::Url &url,
  ...
  static Response<Models::SealAppendBlobResult>
  Seal(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
...
public:
  ...
  static Response<Models::UploadBlockBlobResult>
  Upload(Core::Http::_internal::HttpPipeline &pipeline, const Core::Url &url,
         Core::IO::BodyStream &requestBody,
         const UploadBlockBlobOptions &options, const Core::Context &context);
  ...
  static Response<Models::UploadBlockBlobFromUriResult>
  UploadFromUri(Core::Http::_internal::HttpPipeline &pipeline,
                const Core::Url &url,
  ...
  static Response<Models::StageBlockResult>
  StageBlock(Core::Http::_internal::HttpPipeline &pipeline,
             const Core::Url &url, Core::IO::BodyStream &requestBody,
  ...
  static Response<Models::StageBlockFromUriResult>
  StageBlockFromUri(Core::Http::_internal::HttpPipeline &pipeline,
                    const Core::Url &url,
  ...
  static Response<Models::CommitBlockListResult>
  CommitBlockList(Core::Http::_internal::HttpPipeline &pipeline,
                  const Core::Url &url,
  ...
  static Response<Models::GetBlockListResult>
  GetBlockList(Core::Http::_internal::HttpPipeline &pipeline,
               const Core::Url &url,
