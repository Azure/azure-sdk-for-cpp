...
- [[#7335]](https://github.com/Azure/azure-sdk-for-cpp/issues/7335) The internal properties client no longer terminates the process after a failed close. The service detaches a management link after 30 idle minutes, so a close on that link throws. `Close` left the client marked open after such a throw, and it skipped the session end. The destructor then closed the dead client again, and that second throw escaped a `noexcept` destructor. `Close` now marks the client closed before the call, logs a failed close, and always ends the session. The destructor catches and logs an exception instead of terminating.
