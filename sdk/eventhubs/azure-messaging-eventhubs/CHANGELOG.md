...
- [[#7334]](https://github.com/Azure/azure-sdk-for-cpp/issues/7334) `PartitionClient::ReceiveEvents` now throws a non-transient `EventHubsException` after `Close`. Before this fix, `Close` closed only the AMQP receiver, so the next call blocked until the read deadline and then reported a cancelled operation. A `Close` that runs during a call also stops the recover loop: a call that already holds events returns them, and the next call throws.
