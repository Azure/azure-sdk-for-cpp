...
namespace Azure { namespace Messaging { namespace EventHubs {
      ...
      std::shared_ptr<Azure::Core::Amqp::Models::AmqpMessage const> const& message)
  {
    ...
    {
      Log::Stream(Logger::Level::Informational) << "Batch is full. Cannot add more messages. " << "Message size: " << actualPayloadSize << " size: " << m_currentSize << " Max size: " << m_maxBytes.Value() << std::endl;
      // If we don't have any messages and we can't add this one, then we can't add it at all. Discard the contents of the batch.
      if (m_marshalledMessages.size() == 0)
      {
        m_currentSize = 0;
