...
namespace Azure { namespace Messaging { namespace EventHubs {
  ...
  class ProcessorLoadBalancer {
      ...
      for (auto& ownership : ownerships)
      {
        ...
        if (ownership.OwnerID.empty()
            || std::chrono::duration_cast<std::chrono::minutes> (now
                    - ownership.LastModifiedTime.Value())
                > m_duration)
        {
     ...
     */
    Ownership BalancedLoadBalancer(
      ...
      if (claimMore)
      {
        switch (m_strategy)
        {
          ...
          case ProcessorStrategy::ProcessorStrategyBalanced: {
            ...
            Ownership ownership = BalancedLoadBalancer(lbInfo, ctx);
