...
When setting `AZURE_TEST_MODE=RECORD`, test cases will run the as when running `LIVE`. All the AZURE service network responses are recorded and handled by the test-proxy. Use this test mode to generate pre-recorded data to be used on `PLAYBACK` mode.
